...
#'
#' @param gene_vec Vector of gene symbols or ensemble identifier to convert to entrezID.
...
convert_symb_entrez <- function(gene_vec,
  ...
  if (annotation == "org.Hs.eg.db") {
    ...
    DE_entrezid <- AnnotationDbi::mapIds(x = org.Hs.eg.db::org.Hs.eg.db,
    ...
    # Convert symbols to ENTREZID
    DE_entrezid <- AnnotationDbi::mapIds(x = org.Mm.eg.db::org.Mm.eg.db,
                                         keys = unlist(gene_vec),
                                         column = "ENTREZID",
                                         keytype = "SYMBOL")
